...
void Remote_Init(void)
{
	...
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOD, GPIO_PinSource11);		//将GPIOD.11配置为外部中断模式

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;				 						//PD11 输入
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 								//上拉输入
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOD, &GPIO_InitStructure);
 	GPIO_SetBits(GPIOD,GPIO_Pin_11);															//初始化GPIOD.11
}
